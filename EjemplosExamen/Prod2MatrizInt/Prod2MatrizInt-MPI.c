...
void InitMatrizInt(int *pMatriz1, int *pMatriz2, const int Rows, const int Cols)
{
 for (int i=0;i<Rows*Cols;i++)
 {
 	pMatriz1[i]=lrand48()%10;
 	pMatriz2[i]=lrand48()%10;
 }
}
...
void PrintVectorInt(int *pMatriz, const int Rows, char * String)
{
 printf("%s -----------------------------------------\n", String);
 for (int i=0;i<Rows;i++)
     printf("%2d,", pMatriz[i]);
...
int main(int argc, char **argv)
{
 ...
 gettimeofday(&tv1, &tz);
 #endif
 InitMatrizInt (pMatriz1,pMatriz2,Rows,Cols);
 ...
 PrintVectorInt(pSubMatriz1,pSizeSubV[MyRank],"SubVector1:");
 PrintVectorInt(pSubMatriz2,pSizeSubV[MyRank],"SubVector2:");
 #endif
 ...
 if (MyRank==0)
    {
     #if (PRINT==1)
     PrintVectorInt(pMatriz3,Rows,"Vector3");
     #endif
     free((void *)pMatriz3);
     free((void *)pMatriz2);
     free((void *)pMatriz1);
