/* Fork and join of parallel region outside of 'it'-loop. */ 
/* Calculating abort criterion each 20th iteration only.  */ 
...
# endif
  ...
  itglobal = itmax+1; 
/* iteration */
#pragma omp parallel private(i,k,dphi,dphimax0,it) shared(phi,phin,dx2i,dy2i,dt,dphimax) 
 {
...
#   pragma omp for
    ...
    /* end of omp for */ 
    if ((it % 20) == 0) 
    { /* expensive checking of abort criterion each 20th iteration only*/
      if(dphimax<eps) 
      { 
#       pragma omp single
        { 
