...
#   pragma omp critical
  ...
  */ 
  /* 2nd Solution: declare and calculate num_threads globally */
# ifdef _OPENMP
    omp_set_dynamic(0);
# endif
  num_threads=0;
...
#       pragma omp ordered
  ...
  /* testing the correctness of the numerical result: */
  /* sum_expected = same as in Exa.2 */
  printf("Exa.7: sum  computed=%8.1f,  expected=%8.1f,  difference=%8.5f, num_threads=%2d \n", 
                                 sum,  sum_expected, sum-sum_expected, num_threads); 
