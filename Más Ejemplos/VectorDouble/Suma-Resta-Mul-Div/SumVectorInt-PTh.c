 ...
 for (int i=rows_start; i<rows_end; i++)
	 {
		drand48_r(&RandData,&Random);
      		pVector1[i]=Random*10;
      		pVector2[i]=Random*3*10;
...
//---------------------------------------------------------------------------//
void * SumVectorDoublePTh(void * pTh_Args)
{
 Type_Th_Args * pMyData;
 pMyData=(Type_Th_Args *) pTh_Args;
...
int main(int argc, char **argv)
{
 ...
 pVector1  = (double *) GetMem(Rows,sizeof(double),"Main:Vector1");
 pVector2  = (double *) GetMem(Rows,sizeof(double),"Main:Vector2");
 pVector3  = (double *) GetMem(Rows,sizeof(double),"Main:Vector3");
  ...
  for (int t=0; t<NThreads; t++) 
     {
      ...
      pthread_create(&pThreads[t],NULL,SumVectorDoublePTh, (void *)&pTh_Args[t]);
     }
