 ...
 for (int i=rows_start; i<rows_end; i++)
	 {
		for(int j=0; j<pMyData->Cols; j++){
			   	drand48_r(&RandData,&Random);
      				pMatriz1[i][j]=Random*10;
      				pMatriz2[i][j]=Random*3*10;
	  	}
...
//---------------------------------------------------------------------------//
void * SumMatrizDoublePTh(void * pTh_Args)
{
 Type_Th_Args * pMyData;
 pMyData=(Type_Th_Args *) pTh_Args;
...
int main(int argc, char **argv)
{
 ...
 pMatriz1  = (double **) GetMem2D(Rows,Cols,sizeof(double),"Main:Matriz1");
 pMatriz2  = (double **) GetMem2D(Rows,Cols,sizeof(double),"Main:Matriz2");
 pMatriz3  = (double **) GetMem2D(Rows,Cols,sizeof(double),"Main:Matriz3");
  ...
  for (int t=0; t<NThreads; t++) 
     {
      ...
      pthread_create(&pThreads[t],NULL,SumMatrizDoublePTh, (void *)&pTh_Args[t]);
     }
